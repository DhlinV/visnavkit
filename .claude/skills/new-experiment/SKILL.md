---
name: new-experiment
description: Create and run a training experiment in visnavkit. Use when trying a new hyperparameter, model tweak, or ablation.
---
...
## Steps
1. Copy `visnavkit/configs/experiment/baseline.yaml` to `configs/experiment/<name>.yaml`.
...
3. Smoke it before training (config composes + forward runs):
   ```bash
   uv run python -m visnavkit.scripts.smoke_forward <name>
   ```
