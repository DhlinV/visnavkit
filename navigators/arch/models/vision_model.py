...
class VisionModel(nn.Module):
    def __init__(
        ...
        # for non multi scale feature tasks
        self.img_embed_size = img_embed_size
        self.gap = nn.AdaptiveAvgPool2d(1)
        self.final_linear = nn.Linear(self.embed_dims[-1], self.img_embed_size)
        self.norm1 = nn.BatchNorm1d(self.img_embed_size)
        self.dropout = nn.Dropout(img_embed_drop)

        self.bottleneck_policy = nn.Sequential(
            nn.Dropout(p=neck_cfg.dropout),
            nn.Linear(self.img_embed_size, neck_cfg.dim),
            *[FusableResBlock(neck_cfg.dim, 2 * neck_cfg.dim) for _ in range(neck_cfg.n_res_blocks)],
            nn.Linear(neck_cfg.dim, neck_cfg.dim),
            nn.BatchNorm1d(neck_cfg.dim),
        )
        self.layer_norm = nn.LayerNorm(neck_cfg.dim)
        self.no_bottleneck = nn.Sequential(
            nn.Dropout(p=neck_cfg.dropout),
            nn.Linear(self.img_embed_size, neck_cfg.dim),
            *[FusableResBlock(neck_cfg.dim, 2 * neck_cfg.dim) for _ in range(neck_cfg.n_res_blocks)],
            nn.Linear(neck_cfg.dim, neck_cfg.dim),
    ...
    def forward(self, x, export_heads: list[str] | tuple[str, ...] | None = None):
        ...
        feat_out = self.bottleneck_policy(x)
        feat_out = self.layer_norm(feat_out)

        # heads
        pose = self.pose_head(self.no_bottleneck(x))
...
if __name__ == "__main__":
    # forward smoke test: model/base.yaml vision kwargs, downscaled frame size
