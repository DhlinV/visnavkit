...
def _metadata(path):
    ...
    if metadata.get("onnx_sha256") and metadata["onnx_sha256"] != sha256_file(path):
        raise ValueError("ONNX artifact differs from its metadata. Re-export or supply the matching artifact and sidecar.")
    return metadata
...
def _artifact(cfg, model_id):
    ...
    if cfg.source == "native":
        ...
        export_native(cfg, destination, checkpoint=cfg.checkpoint, seed=cfg.seed,
                      batch_size=cfg.batch_size, model_id=model_id)
        return destination
...
def profile(cfg, model_id):
    if cfg.source == "published" and model_id == "nomad" and cfg.artifact is None:
        ...
        measured = benchmark_nomad(
            cfg.artifact_dir, provider=cfg.runtime.provider, threads=cfg.runtime.threads,
            batch_size=cfg.batch_size, num_samples=cfg.sampling.num_candidates,
            warmup=cfg.runtime.warmup, iterations=cfg.runtime.iterations, seed=cfg.seed, feeds=feeds,
        )
        return {"schema_version": 1, "command": "profile", "model_id": model_id,
                "catalog": asdict(get_model(model_id)), **measured}
    ...
    return {
        "schema_version": 1, "command": "profile", "model_id": model_id,
        ...
        "artifact": str(path), "metadata": meta, "catalog": spec, "runtime": timing, "graph": graph,
    }
...
def evaluate(cfg, model_id):
    ...
    if is_control:
        ...
        if times is None:
            raise ValueError("External evaluation requires explicit adapter.prediction_times_s and metric-space scaling.")
        if meta.get("weights") == "untrained_policy" and dataset_meta.get("dataset_kind") != "synthetic_fixture":
            if not cfg.allow_untrained:
                raise ValueError("Untrained-policy evaluation requires allow_untrained=true; scores are diagnostic only.")
        session = create_session(path, provider=cfg.runtime.provider, threads=cfg.runtime.threads)
        if any(item.shape and isinstance(item.shape[0], int) and item.shape[0] != 1
               for item in session.get_inputs() if item.name != "initial_noise"):
        ...
        if meta.get("selection") == "highest_score" and scores_name is None:
            scores_name = "scores"
        predictor = PREDICTORS.get(adapter.name)(
            session, output_name=adapter.output_name, scores_name=scores_name,
            xy_scale=adapter.xy_scale, layout=adapter.layout,
    ...
    metrics = trajectory_metrics(predictions, data["targets"], times, data["target_times_s"],
                                 scores=scores, horizons=cfg.horizons, valid_mask=data.get("valid_mask"))
    output = Path(cfg.output_dir)
    output.mkdir(parents=True, exist_ok=True)
    np.savez(output / "predictions.npz", trajectories=predictions, prediction_times_s=np.asarray(times),
             sample_ids=data["sample_ids"], **({"scores": scores} if scores is not None else {}))
    synthetic = dataset_meta.get("dataset_kind") == "synthetic_fixture"
    return {
        "schema_version": 1, "command": "evaluate", "model_id": model_id,
        "result_kind": "pipeline_check" if synthetic or meta.get("weights") == "untrained_policy" else "open_loop",
        "dataset": dataset_meta, "dataset_sha256": sha256_file(cfg.dataset_archive),
        "metadata": meta, "adapter": OmegaConf.to_container(adapter, resolve=True),
...
def run(cfg, model_id):
    command = cfg.command
    if command == "list":
        return {"schema_version": 1, "command": command, "models": [asdict(model) for model in list_models()],
                "native_recipes": list(cfg.suite.models), "predictors": PREDICTORS.names()}
    if command == "download":
    ...
    if command == "prepare":
        ...
        return {"schema_version": 1, "command": command, "path": path,
    ...
    if command == "export":
        ...
        return {"schema_version": 1, "command": command, "artifact": str(path),
                "metadata": export_native(cfg, path, checkpoint=cfg.checkpoint, seed=cfg.seed,
                                          batch_size=cfg.batch_size, model_id=model_id)}
    if command == "profile":
    ...
    if command == "smoke":
        ...
        return {"schema_version": 1, "command": command, "result_kind": "pipeline_check",
                "results": [measured, scored]}
    if command == "suite":
        results = []
        ...
        for name in cfg.suite.models:
            try:
                if cfg.source == "native":
                    ...
                    local.model.modules.vision_encoder.pretrained = False
                else:
                    local = copy.deepcopy(cfg)
