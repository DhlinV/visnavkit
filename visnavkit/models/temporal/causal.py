...
def generate_causal_mask(seq_len: int, mask_p: float = 0.0, device=None) -> torch.Tensor:
    ...
    if mask_p > 0.0:
        ...
        causal_mask = causal_mask.masked_fill((rand_mask * lower_triangle).bool(), float("-inf"))
    return causal_mask


class CausalTemporalEncoder(BaseTemporalEncoder):
    """Attend to the current frame and past frames only (safe for per-frame online supervision)."""
