...
from visnavkit.data.dali_augmentations import (
    apply_color_jitter,
    apply_crop,
...
from visnavkit.data.file_list import (
    parse_file_list_frame_ranges,
    resolve_path,
    video_files_from_file_list,
...
class DaliDataset:
    ...
    def __init__(
        ...
        if frame_step < 1 or steps_between_samples < 1:
            raise ValueError("frame_step and steps_between_samples must be positive")
        ...
        safe_rows = [row for row in safe_rows if row[3] - row[2] >= span]
        valid_count = sum(len(range(start, end - span + 1, steps_between_samples)) for _, _, start, end in safe_rows)
        logger.info(
            "Indexed %d windows; discarded %d without complete targets", valid_count, candidate_count - valid_count
        )
        if not safe_rows:
            raise ValueError("No video windows have both observation history and complete future targets")
        self._manifest_dir = tempfile.TemporaryDirectory(prefix="visnavkit-dali-")
        self._reader_file_list = Path(self._manifest_dir.name) / "resolved.txt"
        write_file_list_frame_ranges(safe_rows, self._reader_file_list)
        ...
        for local_shard_id in range(n_streams):
            ...
            self.pipes.append(
                dali_pipeline(
                    file_list=str(self._reader_file_list),
